...
int next_seq_A;
int next_seq_B;
struct pkt packet_expected;

int get_checksum(packet)
struct pkt packet;
{
    int check_sum = 0, i = 0;
    check_sum = packet.checksum;
    check_sum += packet.seqnum;
    check_sum += packet.acknum;
    check_sum = (check_sum >> 16) + (check_sum & 0xffff);
    for (i = 0; i < 20; i += 2) {
      check_sum += (packet.payload[i] << 8) + packet.payload[i+1];
      check_sum = (check_sum >> 16) + (check_sum & 0xffff);
    }
...
void A_output(message) struct msg message;
{
  if (waiting_layer5)
    return;
  memcpy(packet_expected.payload, message.data, sizeof(message.data));
  packet_expected.seqnum = next_seq_A;
  packet_expected.checksum = 0;
  packet_expected.checksum = get_checksum(packet_expected);
  tolayer3(0, packet_expected);
  starttimer(0, 15);
  waiting_layer5 = 1;
}
...
{
  printf("pkt is resent due to A_timerinterupt");
  tolayer3(0, packet_expected);
  starttimer(0, 15);
}  
...
void B_init()
{
/*always has 0 seq becuase B is receiver*/
  next_seq_B=0;
}
